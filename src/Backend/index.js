...
app.get("/api/get", (request, response) => {
  const sqlSelect =
    "SELECT * FROM CrimeReports NATURAL JOIN AreaMapping NATURAL JOIN WeaponsUsed NATURAL JOIN PremisCodes NATURAL JOIN CrimeStatus NATURAL JOIN CrimeCodes limit 10";
...
app.get("/api/get/linegraphdata", (require, response) => {
  const sqlSelect =
    "SELECT CONCAT(month,' ',year) as monthyear, crimecount from (SELECT MONTH(Date_Occ) as month ,DATE_FORMAT(Date_Occ,'%Y') as year, COUNT(*) AS crimecount FROM CrimeReports GROUP BY DATE_FORMAT(Date_Occ,'%Y'), MONTH(Date_Occ) ORDER BY DATE_FORMAT(Date_Occ,'%Y'), MONTH(Date_Occ)) as temp;";
  db.query(sqlSelect, function (err, result, fields) {
    console.log(result);
    response.send(result);
    if (err) throw err;
...
app.post("/api/insert/crime/:maxDRNO", (require, response) => {
  ...
  var s =
    "INSERT INTO CrimeReports VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? )";
  db.query(
    s,
    [
      DR_NO,
...
app.get("/api/search", (request, response) => {
  ...
  let sqlSearch =
    "SELECT * FROM CrimeReports NATURAL JOIN AreaMapping NATURAL JOIN WeaponsUsed NATURAL JOIN PremisCodes NATURAL JOIN CrimeStatus NATURAL JOIN CrimeCodes WHERE 1=1";
  const conditions = [];
...
app.get("/api/get/crimelocations", (require, response) => {
  const sqlSelect =
    "SELECT Latitude, Longitude FROM CrimeReports ORDER BY Date_Occ DESC LIMIT 50;";
  db.query(sqlSelect, function (err, result, fields) {
    console.log(result);
    response.send(result);
    if (err) throw err;
...
app.get("/api/get/unverified", (require, response) => {
  const sqlSelect =
    "SELECT * FROM CrimeReports WHERE Verified = 'Not_Verified';";
  db.query(sqlSelect, function (err, result, fields) {
    console.log(result);
    response.send(result);
    if (err) throw err;
...
app.post("/api/police_login", (req, response) => {
  ...
  const sqlSelect =
    "SELECT * FROM UserLogin WHERE Username = ? AND Password = ? AND AccessControl= 'Police';";
