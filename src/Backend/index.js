...
app.get("/api/get/gendercount", (require, response) => {
  const sqlSelect =
    "SELECT Vict_Sex AS Gender, COUNT(*) AS NumCrimes FROM CrimeReports WHERE Vict_Sex = 'M' OR Vict_Sex = 'F'  GROUP BY Vict_Sex;";
  db.query(sqlSelect, function (err, result, fields) {
    console.log(result);
    response.send(result);
    if (err) throw err;
...
app.put("/api/update/crimereport/", (require, response) => {
  // TODO
  const sqlUpdate =
    "insert ignore into CrimeReports values (" +
    require.body.DR_NO +
    ", " +
    require.body.Date_Rptd +
    ", " +
    require.body.Date_Occ +
    ", " +
    require.body.Time_Occ +
    ", " +
    require.body.Area +
    ", " +
    require.body.Rpt_Dist_No +
    ", " +
    require.body.CrmCd +
    ", " +
    require.body.Vict_Age +
    ", " +
    require.body.Vict_Sex +
    ", " +
    require.body.Vict_Descent +
    ", " +
    require.body.Premis_Cd +
    ", " +
    require.body.Weapon_Used_cd +
    ", " +
    require.body.Status +
    ", " +
    require.body.Location +
    ", " +
    require.body.Latitude +
    ", " +
    require.body.Longitude +
    ", " +
    require.body.Verified +
    ", " +
    require.body.Reported_By +
    ")";
...
app.get("/api/get/user/:userID", (require, response) => {
  const sqlSelect =
    "SELECT * FROM Users natural join UserTeams natural join Players WHERE uID = " +
...
app.get("/api/get/username/:userID", (require, response) => {
  const sqlSelect =
    "SELECT uteamName FROM Users WHERE uID = " + require.params.userID;
  db.query(sqlSelect, (err, result) => {
    response.send(result);
  });
});
...
app.post("/api/modify/player", (require, response) => {
  ...
  var s =
    "update Players set pAttr=?, teamID=?, pName=?, pPos=?, pHeight=?, pWeight=?, overall=?, insideScore=?, outsideScore=?, athleticism=?, playMaking=?, rebounding=?, defending=? where pID=?";
  db.query(
    s,
    [
      pAttr,
...
app.get("/api/get/adv2/:teamNameLike/:arenaNameLike", (require, response) => {
  ...
  const sql =
    '(SELECT arOpenYear, AVG(arCapacity) as capacityBuilt FROM Teams NATURAL JOIN Arenas WHERE teamName LIKE "%' +
    teamNameLike +
    '%" GROUP BY arOpenYear ORDER BY arOpenYear DESC ) UNION (SELECT arOpenYear, AVG(arCapacity) as capacityBuilt FROM Teams NATURAL JOIN Arenas WHERE arName LIKE "%' +
    arenaNameLike +
    '%" GROUP BY arOpenYear ORDER BY arOpenYear DESC )';
  db.query(sql, (err, result) => {
    response.send(result);
    if (err) console.log(error);
  });
